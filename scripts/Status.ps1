...
Show-ServiceStatus -Name "Gateway" -PidFile (Join-Path $projectRoot ".gateway_pid") -Port ([int]$ports.gateway)
...
Write-Host "  Gateway (single port):   http://127.0.0.1:$($ports.gateway)"
Write-Host ""
Write-Host "Tip: Open the Portal URL to access all tabs (Dashboard + AI + Ops)."
Write-Host ""

Write-Host "Recent logs:"
foreach ($name in @("websocket", "kline", "dashboard", "api", "service", "web", "gateway")) {
